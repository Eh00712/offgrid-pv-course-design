...
```
├── README.md                      # 本说明（协作指南）
├── AGENTS.md                      # ⭐ Agent 协作规范（强制，agent 自动读取）
└── docs/                          # 课程设计文档资料
    ├── PROJECT_LOG.md             # 📋 项目日志（每次会话必须更新）
    ├── PROGRESS.md                # 📊 任务进度表（上传进度标准）
...
> 🧠 **如果你（或你的 AI agent）参与本项目，请先读 [`AGENTS.md`](AGENTS.md)** —— 里面定义了必须遵守的协作工作流、项目日志格式、进度表格式和提交信息规范。Claude Code / Codex / Cursor / Copilot 等 agent 打开本仓库时会自动读取它。

核心三条（详见 AGENTS.md）：
1. **每次会话结束必须更新** [`docs/PROJECT_LOG.md`](docs/PROJECT_LOG.md)（做了什么、改了哪些文件、提交 hash）；
2. **任务进度统一记在** [`docs/PROGRESS.md`](docs/PROGRESS.md)，开始前标 🟡、完成后标 ✅；
3. **提交信息必须规范**：`类型(范围): 中文描述`，如 `feat(docs): 添加光伏组件选型表`；只更新日志/进度用 `log:` 类型。

传统（非 agent）协作方式见下节。无论哪种方式，日志与进度表都要更新。
